...
namespace WebPccuClub.DataAccess
{
    ...
    public class BorrowMenuBoardMangDataAccess : BaseAccess
    {

        /// <summary> 查詢結果 </summary>

        public List<BorrowMenuBoardMangResultModel> GetSearchResult(BorrowMenuBoardMangConditionModel model, string SystemCode = null)
        {
...
AND (@ShortDesc IS NULL OR ShortDesc LIKE '%' + @ShortDesc + '%') ";
            ...
            (DbExecuteInfo info, IEnumerable<BorrowMenuBoardMangResultModel> entitys) dbResult = DbaExecuteQuery<BorrowMenuBoardMangResultModel>(CommandText, parameters, true, DBAccessException);

            if (dbResult.info.isSuccess && dbResult.entitys.Count() > 0)
                return dbResult.entitys.ToList();

            return new List<BorrowMenuBoardMangResultModel>();
        }
        ...
        public BorrowMenuBoardMangEditModel GetEditData(string Ser)
        {
            string CommandText = string.Empty;
            DataSet ds = new DataSet();
...
AND (MenuBoardId = @MenuBoardId) ";
            ...
            (DbExecuteInfo info, IEnumerable<BorrowMenuBoardMangEditModel> entitys) dbResult = DbaExecuteQuery<BorrowMenuBoardMangEditModel>(CommandText, parameters, true, DBAccessException);
        ...
        public DbExecuteInfo UpdateData(BorrowMenuBoardMangViewModel vm, UserInfo LoginUser)
        {
