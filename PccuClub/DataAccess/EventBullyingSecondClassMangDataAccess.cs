...
namespace WebPccuClub.DataAccess
{
    ...
    public class EventBullyingSecondClassMangDataAccess : BaseAccess
    {

        /// <summary> 查詢結果 </summary>

        public List<EventBullyingSecondClassMangResultModel> GetSearchResult(EventBullyingSecondClassMangConditionModel model)
        {
...
AND (@Memo IS NULL OR A.Memo LIKE '%' + @Memo + '%') ";
            ...
            (DbExecuteInfo info, IEnumerable<EventBullyingSecondClassMangResultModel> entitys) dbResult = DbaExecuteQuery<EventBullyingSecondClassMangResultModel>(CommandText, parameters, true, DBAccessException);

            if (dbResult.info.isSuccess && dbResult.entitys.Count() > 0)
                return dbResult.entitys.ToList();

            return new List<EventBullyingSecondClassMangResultModel>();
        }
        ...
        public EventBullyingSecondClassMangEditModel GetEditData(string Ser)
        {
            string CommandText = string.Empty;
            DataSet ds = new DataSet();
...
AND (A.ID = @ID) ";
            ...
            (DbExecuteInfo info, IEnumerable<EventBullyingSecondClassMangEditModel> entitys) dbResult = DbaExecuteQuery<EventBullyingSecondClassMangEditModel>(CommandText, parameters, true, DBAccessException);
        ...
        public DbExecuteInfo InsertData(EventBullyingSecondClassMangViewModel vm, UserInfo LoginUser)
        {
        ...
        public DbExecuteInfo UpdateData(EventBullyingSecondClassMangViewModel vm, UserInfo LoginUser)
        {
            DbExecuteInfo ExecuteResult = new DbExecuteInfo();
            DBAParameter parameters = new DBAParameter();
        ...
        public DbExecuteInfo DeletetData(string ser)
        {
            ...
            string CommendText = $@"DELETE FROM EventSecondClassMang WHERE ID = @ID AND CaseSystemType = '03'";
