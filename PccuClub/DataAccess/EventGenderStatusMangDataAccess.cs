...
namespace WebPccuClub.DataAccess
{
    ...
    public class EventGenderStatusMangDataAccess : BaseAccess
    {

        /// <summary> 查詢結果 </summary>

        public List<EventGenderStatusMangResultModel> GetSearchResult(EventGenderStatusMangConditionModel model)
        {
...
AND (@Memo IS NULL OR A.Memo LIKE '%' + @Memo + '%') ";
            ...
            (DbExecuteInfo info, IEnumerable<EventGenderStatusMangResultModel> entitys) dbResult = DbaExecuteQuery<EventGenderStatusMangResultModel>(CommandText, parameters, true, DBAccessException);

            if (dbResult.info.isSuccess && dbResult.entitys.Count() > 0)
                return dbResult.entitys.ToList();

            return new List<EventGenderStatusMangResultModel>();
        }
        ...
        public EventGenderStatusMangEditModel GetEditData(string Ser)
        {
            string CommandText = string.Empty;
            DataSet ds = new DataSet();
...
AND (ID = @ID) ";
            ...
            (DbExecuteInfo info, IEnumerable<EventGenderStatusMangEditModel> entitys) dbResult = DbaExecuteQuery<EventGenderStatusMangEditModel>(CommandText, parameters, true, DBAccessException);
        ...
        public DbExecuteInfo InsertData(EventGenderStatusMangViewModel vm, UserInfo LoginUser)
        {
        ...
        public DbExecuteInfo UpdateData(EventGenderStatusMangViewModel vm, UserInfo LoginUser)
        {
            DbExecuteInfo ExecuteResult = new DbExecuteInfo();
            DBAParameter parameters = new DBAParameter();
        ...
        public DbExecuteInfo DeletetData(string ser)
        {
            ...
            string CommendText = $@"DELETE FROM EventStatusMang WHERE ID = @ID AND CaseSystemType = '02'";
