...
namespace WebPccuClub.DataAccess
{
    ...
    public class BorrowAdminMangDataAccess : BaseAccess
    {

        /// <summary> 查詢結果 </summary>
        public List<BorrowAdminMangResultModel> GetSearchResult(BorrowAdminMangConditionModel model)
        {
...
";
            (DbExecuteInfo info, IEnumerable<BorrowAdminMangResultModel> entitys) dbResult = DbaExecuteQuery<BorrowAdminMangResultModel>(CommandText, parameters, true, DBAccessException);

            if (dbResult.info.isSuccess && dbResult.entitys.Count() > 0)
                return dbResult.entitys.ToList();

            return new List<BorrowAdminMangResultModel>();
        }

        /// <summary>取得編輯資料 </summary>
        public BorrowAdminMangEditModel GetEditData(string LoginId)
        {
            string CommandText = string.Empty;
            DataSet ds = new DataSet();
            ...
            (DbExecuteInfo info, IEnumerable<BorrowAdminMangEditModel> entitys) dbResult = DbaExecuteQuery<BorrowAdminMangEditModel>(CommandText, parameters, true, DBAccessException);
        ...
        public DbExecuteInfo InsertData(BorrowAdminMangViewModel vm, UserInfo LoginUser, string EncryptPw)
        {
        ...
        public DbExecuteInfo InsertRole(BorrowAdminMangViewModel vm, UserInfo LoginUser)
        {
            DbExecuteInfo ExecuteResult = new DbExecuteInfo();
            DBAParameter parameters = new DBAParameter();
            ...
            {
                CommendText = $@"INSERT INTO UserRole (LoginId, RoleId, SystemCode) VALUES (@LoginId, @RoleConsultation, '04')";
        ...
        public DbExecuteInfo UpdateData(BorrowAdminMangViewModel vm, UserInfo LoginUser, string EncryptPw)
        {
            DbExecuteInfo ExecuteResult = new DbExecuteInfo();
            DBAParameter parameters = new DBAParameter();
        ...
        /// <summary> 修改角色資料 </summary>
        public DbExecuteInfo UpdateRole(BorrowAdminMangViewModel vm, UserInfo LoginUser)
        {
            DbExecuteInfo ExecuteResult = new DbExecuteInfo();
            DBAParameter parameters = new DBAParameter();
            ...
            if (!string.IsNullOrEmpty(vm.EditModel.RoleConsultation))
            {
                CommendText = $@"IF EXISTS (SELECT 1 FROM UserRole WHERE 1 = 1 AND LoginId = @LoginId AND SystemCode = '04')
                                    ...
                                    ELSE
                                        BEGIN
                                            INSERT INTO UserRole (LoginId, RoleId, SystemCode) VALUES (@LoginId, @RoleConsultation, '04');
                                        END";
        ...
        public List<BorrowAdminMangResultModel> GetExportResult(BorrowAdminMangConditionModel model)
        {
            string CommandText = string.Empty;
            DataSet ds = new DataSet();
