...
namespace WebPccuClub.DataAccess
{
    ...
    public class EventAdminMangDataAccess : BaseAccess
    {

        /// <summary> 查詢結果 </summary>
        public List<EventAdminMangResultModel> GetSearchResult(EventAdminMangConditionModel model)
        {
...
";
            (DbExecuteInfo info, IEnumerable<EventAdminMangResultModel> entitys) dbResult = DbaExecuteQuery<EventAdminMangResultModel>(CommandText, parameters, true, DBAccessException);

            if (dbResult.info.isSuccess && dbResult.entitys.Count() > 0)
                return dbResult.entitys.ToList();

            return new List<EventAdminMangResultModel>();
        }

        /// <summary>取得編輯資料 </summary>
        public EventAdminMangEditModel GetEditData(string LoginId)
        {
            string CommandText = string.Empty;
            DataSet ds = new DataSet();
            ...
            (DbExecuteInfo info, IEnumerable<EventAdminMangEditModel> entitys) dbResult = DbaExecuteQuery<EventAdminMangEditModel>(CommandText, parameters, true, DBAccessException);
        ...
        public DbExecuteInfo InsertData(EventAdminMangViewModel vm, UserInfo LoginUser, string EncryptPw)
        {
        ...
        public DbExecuteInfo InsertRole(EventAdminMangViewModel vm, UserInfo LoginUser)
        {
            DbExecuteInfo ExecuteResult = new DbExecuteInfo();
            DBAParameter parameters = new DBAParameter();
            ...
            {
                CommendText = $@"INSERT INTO UserRole (LoginId, RoleId, SystemCode) VALUES (@LoginId, @RoleConsultation, '03')";
        ...
        public DbExecuteInfo UpdateData(EventAdminMangViewModel vm, UserInfo LoginUser, string EncryptPw)
        {
            DbExecuteInfo ExecuteResult = new DbExecuteInfo();
            DBAParameter parameters = new DBAParameter();
        ...
        /// <summary> 修改角色資料 </summary>
        public DbExecuteInfo UpdateRole(EventAdminMangViewModel vm, UserInfo LoginUser)
        {
            DbExecuteInfo ExecuteResult = new DbExecuteInfo();
            DBAParameter parameters = new DBAParameter();
            ...
            if (!string.IsNullOrEmpty(vm.EditModel.RoleConsultation))
            {
                CommendText = $@"IF EXISTS (SELECT 1 FROM UserRole WHERE 1 = 1 AND LoginId = @LoginId AND SystemCode = '03')
                                    ...
                                    ELSE
                                        BEGIN
                                            INSERT INTO UserRole (LoginId, RoleId, SystemCode) VALUES (@LoginId, @RoleConsultation, '03');
                                        END";
        ...
        public List<EventAdminMangResultModel> GetExportResult(EventAdminMangConditionModel model)
        {
            string CommandText = string.Empty;
            DataSet ds = new DataSet();
